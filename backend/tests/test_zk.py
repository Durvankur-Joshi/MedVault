...
# --- TEST 2: Wrong Secret / Invalid Proof Rejection ---
def test_invalid_zk_proof_rejected(client, db):
    ...
    # Tampered/invalid proof bytes
    invalid_proof = "0x" + "00" * 64
    ...
    assert "mismatch" in v_resp.json()["details"].lower() or "failed" in v_resp.json()["details"].lower()
...
def test_tampered_record_commitment_rejected(client, db):
    ...
    # Wrong record commitment length or modified bytes
...
def test_tampered_authorization_commitment_rejected(client, db):
    ...
    tampered_auth_commit = "0x" + "ff" * 31  # invalid length
...
# --- TEST 5: Zero PII in Public Inputs ---
...
# --- TEST 6: Patient Cannot Generate Doctor Authorization Proof ---
...
# --- TEST 7: Doctor Cannot Generate Proof Without Active Consent ---
...
# --- TEST 8: Decrypted Retrieval Seamlessly Verifies ZK Proof ---
